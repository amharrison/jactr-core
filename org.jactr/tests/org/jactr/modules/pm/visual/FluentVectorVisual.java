  ...
  public IModel get()
  {
    try
    {
      ...
      IChunkType sequence = FluentChunkType.from(model)
          .slots("src", "dst", "expected", "next").encode();

      FluentChunk.from(visLoc).slot(IVisualModule.SCREEN_X_SLOT, 0)
          .slot(IVisualModule.SCREEN_Y_SLOT, 0).encode();
      IChunk upperLeft = FluentChunk.from(visLoc)
          .slot(IVisualModule.SCREEN_X_SLOT, -60)
          .slot(IVisualModule.SCREEN_Y_SLOT, 45).encode();
      IChunk lowerLeft = FluentChunk.from(visLoc)
          .slot(IVisualModule.SCREEN_X_SLOT, -60)
          .slot(IVisualModule.SCREEN_Y_SLOT, -45).encode();
      IChunk upperRight = FluentChunk.from(visLoc)
          .slot(IVisualModule.SCREEN_X_SLOT, 60)
          .slot(IVisualModule.SCREEN_Y_SLOT, 45).encode();
      IChunk lowerRight = FluentChunk.from(visLoc)
          .slot(IVisualModule.SCREEN_X_SLOT, 60)
          .slot(IVisualModule.SCREEN_Y_SLOT, -45).encode();
      FluentChunk.from(visLoc).slot(IVisualModule.SCREEN_X_SLOT, 0)
          .slot(IVisualModule.SCREEN_Y_SLOT, -45).encode();
      ...
      IChunk sequence4 = FluentChunk.from(sequence).slot("src", upperRight)
          .slot("dst", upperLeft).slot("expected", null).slot("next", null)
          .encode();
      IChunk sequence3 = FluentChunk.from(sequence).slot("src", lowerLeft)
          .slot("dst", upperRight).slot("expected", "center")
          .slot("next", sequence4).encode();
      IChunk sequence2 = FluentChunk.from(sequence).slot("src", lowerRight)
          .slot("dst", lowerLeft).slot("expected", "lowerMiddle")
          .slot("next", sequence3).encode();
      ...
      FluentProduction.from(model).named("search-between")
          .condition(FluentCondition.match("goal", goal)
              .slot("stage", constants.get("retrieve")).build())
          .condition(FluentCondition.match("retrieval", sequence)
              .slot("src", "=src").slot("dst", "=dst").build())
          .action(FluentAction.modify("goal")
          ...
          .action(FluentAction.add("visual-location", visLoc)
              ...
              .slot(":vector-threshold", 0.5).build())
