...
/**
 * duped from FluentVisual, productions just have vocal actions.
 * 
 * @author harrison
 */
public class FluentVocal implements Supplier<IModel>
{
  ...
  public IModel get()
  {
    try
    {
      ...
      IChunkType speak = model.getDeclarativeModule().getChunkType("speak")
          .get();
      ...
      FluentProduction.from(model).named("search-kind")
          ...
          .condition(FluentCondition.query("vocal").slot("state", free).build())
          .action(FluentAction.add("vocal", speak)
              .slot("string", "say something").build())
