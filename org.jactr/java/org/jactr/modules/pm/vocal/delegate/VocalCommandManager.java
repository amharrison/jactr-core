...
public class VocalCommandManager
    extends EfferentCommandManager<VocalizationCommand>
{
  /**
   * Logger definition
   */
  ...
  protected void commandAccepted(VocalizationCommand command)
  {
    ...
    if (LOGGER.isDebugEnabled())
      LOGGER.debug("vocalization of " + command.getText() + " accepted");
    if (_module.hasListeners()) _module.dispatch(
        new VocalModuleEvent(_module, VocalModuleEvent.Type.PREPARED, command));
  }
  ...
  protected void commandRejected(VocalizationCommand command)
  {
    _isPreparing = false;
    if (LOGGER.isDebugEnabled())
      LOGGER.debug("vocalization of " + command.getText() + " rejected");
  }
  ...
  {
    if (LOGGER.isDebugEnabled())
      LOGGER.debug("vocalization of " + command.getText() + " running");

    if (!_firedStart) if (_module.hasListeners()) _module.dispatch(
        new VocalModuleEvent(_module, VocalModuleEvent.Type.STARTED, command));
  ...
  {
    if (LOGGER.isDebugEnabled())
      LOGGER.debug("vocalization of " + command.getText() + " aborted");

    if (_firedStart)
      if (_module.hasListeners()) _module.dispatch(new VocalModuleEvent(_module,
          VocalModuleEvent.Type.COMPLETED, command));
  ...
  {
    if (LOGGER.isDebugEnabled())
      LOGGER.debug("vocalization of " + command.getText() + " completed");
  ...
  @Override
  protected VocalizationCommand createCommand(
      IEfferentObject vocalizationSource, Object... parameters)
  {
    for (IEfferentCommandTemplate<?> template : vocalizationSource
        .getCommandTemplates())
      if (template instanceof VocalizationCommandTemplate) try
      {
        ...
        command.setProperty(IEfferentCommand.ESTIMATED_DURATION, parameters[1]);
        command.setIsSubvocalization((Boolean) parameters[2]);
      ...
      {
        throw new RuntimeException("Could not create vocalization command ", e);
      }
