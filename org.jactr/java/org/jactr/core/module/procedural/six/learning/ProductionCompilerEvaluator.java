...
public class ProductionCompilerEvaluator
{
  ...
  // in a row ) and can't compile out
  ...
  private boolean isRetrievalTypeBuffer(ICompilableContext context,
      IRequest req)
  {
    if (!context.isImmediate(req) && !context.isDeterministic(req)
        && !context.isJammable(req) && context.canCompileOut(req))
      ...
      ICompilableBuffer buffer)
  {
    ...
    if (isSimpleType(buffer.getCompilableContext(), one.getIRequest())) // LOGGER.debug("checking
                                                                        ...
                                                                        // +
                                                                        // two.index);
      return simpleEvaluatorTable[one.index][two.index].canCompile(one, two,
          buffer.getName());
    if (isRetrievalTypeBuffer(buffer.getCompilableContext(), one.getIRequest())) // LOGGER.debug("checking
                                                                                 // retrieval
                                                                                 // type
                                                                                 // compilation
                                                                                 // map
    ...
    if (isPerceptualTypeBuffer(buffer.getCompilableContext(),
        one.getIRequest())) // LOGGER.debug("checking perceptual type
                            ...
                            // two.index);
      return perceptualEvaluatorTable[one.index][two.index].canCompile(one, two,
          buffer.getName());

    return false;
  }

  public Map<String, Object> extractMap(BufferStruct one, BufferStruct two,
      ICompilableBuffer buffer)
  {
    if (isSimpleType(buffer.getCompilableContext(), one.getIRequest())) // LOGGER.debug("checking
                                                                        // goal
                                                                        ...
                                                                        // +
                                                                        // two.index);
      return simpleMapperTable[one.index][two.index].extractMap(one, two,
          buffer.getName());
    ...
    if (isPerceptualTypeBuffer(buffer.getCompilableContext(),
        one.getIRequest())) // LOGGER.debug("checking perceptual type
                            // compilation map for indices " + one.index + "," +
                            // two.index);
      return perceptualMapperTable[one.index][two.index].extractMap(one, two,
          buffer.getName());
    return null;
  }

  public BufferStruct collapseBuffer(BufferStruct one, BufferStruct two,
      ICompilableBuffer buffer)
  {
    if (isSimpleType(buffer.getCompilableContext(), one.getIRequest()))
      return simpleCollapserTable[one.index][two.index].collapseBuffer(one, two,
          buffer.getName());
    ...
    if (isPerceptualTypeBuffer(buffer.getCompilableContext(),
        one.getIRequest()))
      return perceptualCollapserTable[one.index][two.index].collapseBuffer(one,
          two, buffer.getName());
    return null;
  }
  ...
  public abstract static class Evaluator
  {
    ...
    static public Evaluator TRUE  = new Evaluator() {
                                    @Override
                                    public boolean canCompile(BufferStruct one,
                                        BufferStruct two, String bufferName)
                                    {
                                      // LOGGER.debug("Evaluator returning true
                                      // by default.");
                                      return true;
                                    }
                                  };
    ...
    static public Evaluator FALSE = new Evaluator() {
                                    ...
                                    public boolean canCompile(BufferStruct one,
                                        BufferStruct two, String bufferName)
                                    {
                                      // LOGGER.debug("Evaluator returning false
                                      // by default.");
                                      return false;
                                    }
                                  };
  ...
  public abstract static class Mapper
  {
    ...
    static public Mapper ERROR = new Mapper() {
                                 ...
                                 public Map<String, Object> extractMap(
                                     BufferStruct one, BufferStruct two,
                                     String bufferName)
                                 {
                                   return null;
                                 }
    ...
    static public Map<String, Object> match(Collection<? extends ISlot> slots1,
        Collection<? extends ISlot> slots2, VariableBindings instantiationTwo)
    {
      ...
      for (ISlot s2 : slots2)
        for (ISlot s1 : slots1)
          ...
          if (s1.getName().equals(s2.getName()) && ((IConditionalSlot) s1)
              .getCondition() == ((IConditionalSlot) s2).getCondition())
  ...
  public abstract static class Collapser
  {
    ...
    static public Collapser EMPTY = new Collapser() {
                                    ...
                                    public BufferStruct collapseBuffer(
                                        ...
                                        String bufferName)
                                    {
                                      return new BufferStruct(bufferName,
                                          one.hasStrictHarvesting(), null);
                                    }
                                  };

    static public void findQueryCondition(BufferStruct one, BufferStruct two,
        BufferStruct newStruct)
    {
      ...
      for (IAction a : one.getActions())
        if (a instanceof AddAction) addAction = true;
      ...
      // at this point we know whether there is an add action/query in #2 and
      // that there is def. not a query in buffer one.
      if (secondQuery == null) return;

      // now, def. no add action and no query condition in #1 but query
      // condition in #2
    ...
    static public boolean minusSlots(ISlotContainer sc1, ISlotContainer sc2,
        ISlotContainer result)
    {
      slots.clear();
      ...
      for (ISlot s1 : slots)
      {
        ...
        if (sc2 != null) for (ISlot s2 : sc2.getSlots())
        {
          ...
          {
            LOGGER.warn(
                "Encountered unexpected case where action assignment function is not equals... not compiling "
                    + sc1);
            return false;
          }
          if (s1.getName().equals(s2.getName()) && ((IConditionalSlot) s1)
              .getCondition() == IConditionalSlot.EQUALS) // both are equals
                                                          // functions. match up
                                                          // variables.
            mentioned = true;
        }
    ...
    static public boolean calcNewCondition(ISlotContainer c1, ISlotContainer c2,
        ISlotContainer a1, ISlotContainer newCondition)
    {
    ...
    static public boolean calcNewAction(ISlotContainer a1, ISlotContainer a2,
        ISlotContainer newAction)
    {
      // newA = A1 overwritten by A2
      // LOGGER.debug("calculating new action from " + a1.getSlots() + " and " +
      // a2.getSlots());
      slots.clear();
      a2.getSlots(slots);
      for (ISlot s : slots)
      ...
      if (minusSlots(a1, a2, newAction)) // LOGGER.debug(" and once added in
                                         // from first action " +
                                         // newAction.getSlots());
        return true;
      return false;
    ...
    static public BufferStruct cloneProduction(BufferStruct toClone,
        String bufferName)
    {
      BufferStruct newStruct = new BufferStruct(bufferName,
          toClone.hasStrictHarvesting(), toClone.getVariableBindings());

      for (ICondition condition : toClone.getConditions())
      {
        ICondition cloned = clone(condition);
        if (cloned != null)
          newStruct.conditions.add(cloned);
        else
          return null;
      }
    ...
    static public ICondition clone(ICondition condition)
    {
      if (condition instanceof QueryCondition)
        ...
        return new VariableCondition(
            ((VariableCondition) condition).getBufferName(),
            ((VariableCondition) condition).getVariableName());
      else if (condition instanceof ChunkTypeCondition
          && !(condition instanceof ChunkCondition))
        return new ChunkTypeCondition(
            ((ChunkTypeCondition) condition).getBufferName(),
            ((ChunkTypeCondition) condition).getChunkType(),
            ((ChunkTypeCondition) condition).getSlots());
      else
      {
        LOGGER.debug("don't know how to clone/handle condition of instance "
            + condition);
        return null;
      }
    }
    ...
    static public IAction clone(IAction action)
    {
      if (action instanceof RemoveAction)
        ...
        return new ModifyAction(((ModifyAction) action).getBufferName(),
            ((ModifyAction) action).getSlots());
      else if (action instanceof AddAction)
        return new AddAction(((AddAction) action).getBufferName(),
            ((AddAction) action).getReferant(),
            ((AddAction) action).getSlots());
      else if (action instanceof SetAction)
        return new SetAction(((SetAction) action).getBufferName(),
            ((SetAction) action).getReferant(),
            ((SetAction) action).getSlots());
      else
      {
        LOGGER.debug(
            "don't know how to clone/handle action of instance " + action);
        return null;
      }
    }
  }
  ...
  static public Evaluator no_rhs_reference                           = new Evaluator() {
                                                                           ...
                                                                           String bufferName)
                                                                       {
                                                                         ...
                                                                         // of
                                                                         // p2
                                                                         for (IAction action : two
                                                                             .getActions())
                                                                           if (action instanceof ModifyAction)
                                                                           {
                                                                             for (ISlot s : ((ModifyAction) action)
                                                                                 .getSlots())
                                                                               if (s
                                                                                   .getValue()
                                                                                   .toString()
                                                                                       ...
                                                                                       "=" + bufferName))
                                                                               {
                                                                                 LOGGER
                                                                                     .debug(
                                                                                         "Evaluator no_rhs_reference returning false because of action "
                                                                                             + action
                                                                                             + " and slot "
                                                                                             + s);
                                                                                 return false;
                                                                         ...
                                                                         for (ICondition condition : two
                                                                           ...
                                                                           if (condition instanceof QueryCondition
                                                                               && ((QueryCondition) condition)
                                                                                   ...
                                                                                   .equals(
                                                                                       bufferName))
                                                                             for (ISlot s : ((QueryCondition) condition)
                                                                                 .getSlots())
                                                                             if (s
                                                                                 .getName()
                                                                                 .equals(
                                                                                     "state"))
                                                                             {
                                                                                       ...
                                                                                       "busy"))
                                                                               {
                                                                                 LOGGER
                                                                                     .debug(
                                                                                         "Evaluator busy_or_ProductionCompiler6.empty returning false because of condition "
                                                                                             + condition
                                                                                             + " and slot "
                                                                                             + s);
                                                                                 return false;
  ...
  static public Evaluator same_queries                               = new Evaluator() {
                                                                           ...
                                                                           String bufferName)
                                                                       {
                                                                         ...
                                                                         for (ISlot s1 : query1)
                                                                         {
                                                                           ...
                                                                           // here,
                                                                           // no
                                                                           // ProductionCompiler6.match!!
                                                                           LOGGER
                                                                               .debug(
                                                                                   "Evaluator same_queries returning false because of unmatched slot "
                                                                                       + s1
                                                                                       + " from production one");
                                                                           return false;
                                                                         }
  ...
  /*
   * @Deprecated //should be handeled by mapper_no_add_p1... static public
   * Mapper mapper_match_and_match = new Mapper() { public Map<String, Object>
   * extractMap(BufferStruct one, BufferStruct two, String bufferName) { //just
   * match the variables from the conditions of the first one to the conditions
   * of the second one. if(one.getConditions().size() > 1 ||
   * two.getConditions().size() > 1) { LOGGER.
   * warn("Should not be here in ProductionCompilerEvaluator.Mapper.matches_and_match; more than one condition for one of the productions."
   * + one.getConditions() + "," + two.getConditions()); return null; } return
   * Mapper.match(((ISlotContainer)one.conditions.iterator().next()).getSlots(),
   * ((ISlotContainer)two.conditions.iterator().next()).getSlots(),
  ...
  static public Mapper    mapper_simple_no_add_p1                    = new Mapper() {
                                                                           ...
                                                                           String bufferName)
                                                                       {
                                                                                 ...
                                                                                 .size() > 1)
                                                                         {
                                                                           LOGGER
                                                                               .warn(
                                                                                   "Should not be here in ProductionCompilerEvaluator.Mapper.match_no_add_p1; more than one condition or action for one of the productions."
                                                                                       + one
                                                                                           .getConditions()
                                                                                       + ","
                                                                                       + one
...
//new version of the above; because we now support match/queries in P1 with other actual stuff in P2, we need this one
  ...
  static public Mapper    mapper_add_p1                              = new Mapper() {
                                                                           ...
                                                                           String bufferName)
                                                                       {
                                                                         ...
                                                                         for (ICondition condition : two
                                                                           ...
                                                                           if (!(condition instanceof QueryCondition))
                                                                           {
                                                                             if (conditionTwo != null)
                                                                             {
                                                                               LOGGER
                                                                                   .warn(
                                                                                       "somehow got to mapper_add_p1 incorrectly... more than one non-query conditionTwo "
                                                                                           + two
                                                                                               .getConditions());
                                                                               return null;
                                                                             }
                                                                         ...
                                                                         if (add1 == null)
                                                                         {
                                                                           LOGGER
                                                                               .warn(
                                                                                   "Should not be here in ProductionCompilerEvaluator.Mapper.match_add_p1; no add action."
                                                                                       + one
                                                                                           .getConditions()
                                                                                       + ","
                                                                                       + one
  ...
  // Not sure this needs to be different from mapper_add_p1, but it is in lisp's
  // ACT-R documentation, so I'll leave it for now.
  static public Mapper    mapper_nondeterministic_add_p1             = new Mapper() {
                                                                           ...
                                                                           String bufferName)
                                                                       {
                                                                         ...
                                                                         for (IAction a : one
                                                                                 ...
                                                                                 .getSlots())
                                                                           {
                                                                                     ...
                                                                                     "="))
                                                                             {
                                                                               LOGGER
                                                                                   .warn(
                                                                                       "variable slot name in retrieval add action; can't handle "
                                                                                           + a
                                                                                           + ","
                                                                                           + s);
                                                                               return null;
                                                                         ...
                                                                         for (ICondition c : two
                                                                             .getConditions())
                                                                           if (c instanceof ChunkTypeCondition)
                                                                             for (ISlot s : ((ChunkTypeCondition) c)
                                                                                 .getSlots())
                                                                             if (s
                                                                                 .isVariableValue())
                                                                               map.put(
                                                                           ...
                                                                           String bufferName)
                                                                       {
                                                                         ...
                                                                         for (ICondition condition : one
                                                                           ...
                                                                           if (!(condition instanceof QueryCondition))
                                                                           {
                                                                             if (conditionOne != null)
                                                                             {
                                                                               LOGGER
                                                                                   .warn(
                                                                                       "something is wrong in collapse_no_add_in_p1... more than one non-query conditionOne.");
                                                                               return null;
                                                                             }
                                                                             conditionOne = condition;
                                                                           }
                                                                         for (ICondition condition : two
                                                                           ...
                                                                           if (!(condition instanceof QueryCondition))
                                                                           {
                                                                             if (conditionTwo != null)
                                                                             {
                                                                               LOGGER
                                                                                   .warn(
                                                                                       "something is wrong in collapse_no_add_in_p1... more than one non-query conditionTwo.");
                                                                               return null;
                                                                             }
                                                                             conditionTwo = condition;
                                                                           }
                                                                             ...
                                                                             .size() > 0)
                                                                         {
                                                                           ...
                                                                           if (conditionOne != null
                                                                               || conditionTwo != null)
                                                                           {
                                                                             ChunkTypeCondition match = new ChunkTypeCondition(
                                                                                 bufferName,
                                                                                 ((ChunkTypeCondition) conditionOne)
                                                                                     .getChunkType());
                                                                             if (!calcNewCondition(
                                                                                 (ISlotContainer) conditionOne,
                                                                                 (ISlotContainer) conditionTwo,
                                                                         ...
                                                                         else
                                                                         {
                                                                           ...
                                                                           if (conditionOne != null
                                                                               || conditionTwo != null)
                                                                           {
                                                                             ChunkTypeCondition match = conditionOne != null
                                                                                 ? new ChunkTypeCondition(
                                                                                     bufferName,
                                                                                     ((ChunkTypeCondition) conditionOne)
                                                                                         .getChunkType())
                                                                                 : new ChunkTypeCondition(
                                                                                     bufferName,
                                                                                     ((ChunkTypeCondition) conditionTwo)
                                                                                         .getChunkType());
                                                                             if (!calcNewCondition(
                                                                                 (ISlotContainer) conditionOne,
                                                                                 (ISlotContainer) conditionTwo,
  ...
  // we can assume this won't ever get called if strict harvesting is relevant /
  // enabled.
                                                                           ...
                                                                           String bufferName)
                                                                       {
                                                                         ...
                                                                         for (IAction action : two
                                                                           ...
                                                                           else
                                                                           {
                                                                             LOGGER
                                                                                 .warn(
                                                                                     "encountered unhandled case of non-modify action "
                                                                                         + action);
                                                                             return null;
                                                                           }
                                                                         ...
                                                                         if (plusOne == null)
                                                                         {
                                                                           LOGGER
                                                                               .warn(
                                                                                   "encountered unhandled case of no plus case in p1 "
                                                                                       + one
                                                                                           .getActions());
                                                                           return null;
                                                                         }
                                                                         if (modTwo != null)
                                                                         {
                                                                           AddAction add = new AddAction(
                                                                               ...
                                                                               plusOne
                                                                                   .getChunkType());
                                                                           if (!calcNewAction(
                                                                               (ISlotContainer) plusOne,
                                                                               (ISlotContainer) modTwo,
                                                                               add))
                                                                             return null;
                                                                           newStruct.actions
                                                                               .add(
  ...
  /*
   * @Deprecated static public Collapser collapse_add_p1_check_sh = new
   * Collapser() { public BufferStruct collapseBuffer(BufferStruct one,
   * BufferStruct two, String bufferName) { if(one.hasStrictHarvesting()) {
   * for(Iterator<IAction> it = one.getActions().iterator(); it.hasNext(); ) {
   * IAction action = it.next(); if(action instanceof AddAction) { it.remove();
   * } } two.getActions().add(new RemoveAction(bufferName)); return
   * collapse_no_add_p1.collapseBuffer(one, two, bufferName); } else return
   * collapse_add_p1.collapseBuffer(one, two, bufferName); } };
   */

  /*
   * @Deprecated //sh stands for "strict harvesting //according to tony, this is
   * deprecated static public Collapser collapse_no_add_p1_check_sh = new
   * Collapser() { public BufferStruct collapseBuffer(BufferStruct one,
   * BufferStruct two, String bufferName) { if(one.hasStrictHarvesting()) {
   * one.getActions().add(new RemoveAction(bufferName)); } return
   * collapse_no_add_p1.collapseBuffer(one, two, bufferName); } };
   */

  /*
   * @Deprecated //if something in retrieval from our add, and no strict
   * harvesting, we can't combine static public Collapser
   * collapse_retrieval_add_p1_check_sh = new Collapser() { public BufferStruct
   * collapseBuffer(BufferStruct one, BufferStruct two, String bufferName) {
   * if(!one.hasStrictHarvesting()) return null; return
   * collapse_nondeterministic_add_p1.collapseBuffer(one, two, bufferName); } };
   */
                                                                           ...
                                                                           String bufferName)
                                                                       {
                                                                         ...
                                                                         for (ICondition condition : two
                                                                           ...
                                                                           if (!(condition instanceof QueryCondition))
                                                                           {
                                                                             LOGGER
                                                                                 .warn(
                                                                                     "somehow got to collapse_retrieval_add_p1_not_harvested incorrectly... got a non-query conditionTwo "
                                                                                         + two
                                                                                             .getConditions());
                                                                             return null;
                                                                           }
  ...
  private void initializeMaps(Integer bufferActions[])
  {
    ...
    simpleEvaluatorTable = new Evaluator[(int) (Math.pow(2,
    ...
    perceptualEvaluatorTable = new Evaluator[(int) (Math.pow(2,
        bufferActions.length) - 1)][(int) Math.pow(2, bufferActions.length)
            - 1];
    simpleMapperTable = new Mapper[(int) (Math.pow(2, bufferActions.length)
    ...
    for (int i = 0; i < tableArray.length; i++)
    {
      ...
      tableArray[i][match + modify + add + remove] = tableArray[i][match
          + modify + add];
      tableArray[i][query + add + remove] = tableArray[i][query + add];
      tableArray[i][match + query + add + remove] = tableArray[i][match + query
          + add];
      tableArray[i][match + query + modify + remove + add] = tableArray[i][match
          + query + modify + add];
      ...
      for (int j = 0; j < Math.pow(2, bufferActions.length) - 1; j++)
      {
        ...
        tableArray[i][j][match + modify + add + remove] = tableArray[i][j][match
            + modify + add];
        tableArray[i][j][query + add + remove] = tableArray[i][j][query + add];
        tableArray[i][j][match + query + add + remove] = tableArray[i][j][match
            + query + add];
        tableArray[i][j][match + query + modify + remove
            + add] = tableArray[i][j][match + query + modify + add];
      }
    }

    // now begins the long, arduous process of filling these in... not the
    ...
    simpleEvaluatorTable[match + modify][match + modify
        + remove] = Evaluator.TRUE;
    ...
    simpleCollapserTable[match + modify][match + modify
        + add] = collapse_no_add_p1;
    simpleCollapserTable[match + modify][match + modify
        + remove] = collapse_no_add_p1;
    ...
    simpleEvaluatorTable[match + modify + add][match
        + modify] = no_rhs_reference;
    ...
    simpleCollapserTable[match + modify + add][match
        + modify] = collapse_add_p1;
    ...
    retrievalEvaluatorTable[add][query] = p2_busy_or_empty; // not yet
                                                            // harvested;
                                                            // otherwise create
                                                            // a situation where
                                                            // the p2 fires once
    ...
    retrievalEvaluatorTable[add][add + query] = Evaluator.TRUE; // FIXME: not
                                                                // sure this is
                                                                // right, but ok
                                                                // for now...
    ...
    retrievalMapperTable[add][match + add
        + query] = mapper_nondeterministic_add_p1;
    retrievalMapperTable[add][match + remove
        + query] = mapper_nondeterministic_add_p1; // tested

    retrievalCollapserTable[add][empty] = collapse_nonimmediate_add_p1_not_harvested;
    retrievalCollapserTable[add][match] = collapse_add_p1;// collapse_cancompileout_add_p1;
                                                          // -- this is wrong
                                                          // because at the end
                                                          // of this there is
                                                          // still a chunk in
    ...
    retrievalCollapserTable[add][match + remove
        + query] = collapse_compileout_add_p1; // tested
    ...
    retrievalMapperTable[match + add][query] = Mapper.EMPTY;
    retrievalMapperTable[match + add][match
        + add] = mapper_nondeterministic_add_p1;
    retrievalMapperTable[match + add][match
        + remove] = mapper_nondeterministic_add_p1;
    retrievalMapperTable[match + add][match
        + query] = mapper_nondeterministic_add_p1;
    retrievalMapperTable[match + add][add + query] = Mapper.EMPTY;
    retrievalMapperTable[match + add][match + add
        + query] = mapper_nondeterministic_add_p1;

    retrievalCollapserTable[match
        + add][empty] = collapse_nonimmediate_add_p1_not_harvested;
    retrievalCollapserTable[match + add][match] = collapse_add_p1; // don't use
                                                                   // compile
                                                                   // out
                                                                   // version
                                                                   // since at
    ...
    retrievalCollapserTable[match
        + add][query] = collapse_nonimmediate_add_p1_not_harvested;
    retrievalCollapserTable[match + add][match
        + add] = collapse_compileout_add_p1;
    retrievalCollapserTable[match + add][match
        + remove] = collapse_compileout_add_p1;
    retrievalCollapserTable[match + add][match + query] = collapse_add_p1;
    retrievalCollapserTable[match + add][add
    ...
    retrievalEvaluatorTable[add + query][match + remove
        + query] = Evaluator.TRUE; // tested
    ...
    retrievalMapperTable[add + query][match
        + add] = mapper_nondeterministic_add_p1;
    retrievalMapperTable[add + query][match
        + remove] = mapper_nondeterministic_add_p1; // tested
    retrievalMapperTable[add + query][match
        + query] = mapper_nondeterministic_add_p1;
    retrievalMapperTable[add + query][add + query] = Mapper.EMPTY;
    retrievalMapperTable[add + query][match + add
        + query] = mapper_nondeterministic_add_p1; // tested
    retrievalMapperTable[add + query][match + remove
        + query] = mapper_nondeterministic_add_p1; // tested

    retrievalCollapserTable[add
        + query][empty] = collapse_nonimmediate_add_p1_not_harvested;
    retrievalCollapserTable[add + query][match] = collapse_add_p1;
    retrievalCollapserTable[add + query][add] = collapse_compileout_add_p1;
    retrievalCollapserTable[add
        + query][query] = collapse_nonimmediate_add_p1_not_harvested;
    ...
    retrievalCollapserTable[add + query][add
        + query] = collapse_compileout_add_p1;
    retrievalCollapserTable[add + query][match + add
        + query] = collapse_compileout_add_p1; // tested
    retrievalCollapserTable[add + query][match + remove
        + query] = collapse_compileout_add_p1; // tested
    ...
    retrievalEvaluatorTable[match + add + query][match
        + remove] = Evaluator.TRUE; // tested
    retrievalEvaluatorTable[match + add + query][match
        + query] = Evaluator.TRUE;
    retrievalEvaluatorTable[match + add + query][add
        + query] = p2_busy_or_empty;
    retrievalEvaluatorTable[match + add + query][match + add
        + query] = Evaluator.TRUE;

    retrievalMapperTable[match + add + query][empty] = Mapper.EMPTY;
    retrievalMapperTable[match + add
        + query][match] = mapper_nondeterministic_add_p1;
    retrievalMapperTable[match + add + query][add] = Mapper.EMPTY;
    retrievalMapperTable[match + add + query][query] = Mapper.EMPTY;
    retrievalMapperTable[match + add + query][match
        + add] = mapper_nondeterministic_add_p1;
    retrievalMapperTable[match + add + query][match
        + remove] = mapper_nondeterministic_add_p1; // tested
    retrievalMapperTable[match + add + query][match
    ...
    retrievalMapperTable[match + add + query][match + add
        + query] = mapper_nondeterministic_add_p1;

    retrievalCollapserTable[match + add
        + query][empty] = collapse_nonimmediate_add_p1_not_harvested;
    retrievalCollapserTable[match + add + query][match] = collapse_add_p1;
    retrievalCollapserTable[match + add
        + query][add] = collapse_compileout_add_p1;
    retrievalCollapserTable[match + add
        + query][query] = collapse_nonimmediate_add_p1_not_harvested;
    retrievalCollapserTable[match + add + query][match
        + add] = collapse_compileout_add_p1;
    retrievalCollapserTable[match + add + query][match
        + remove] = collapse_compileout_add_p1; // tested
    retrievalCollapserTable[match + add + query][match
        + query] = collapse_add_p1;
    retrievalCollapserTable[match + add + query][add
        + query] = collapse_compileout_add_p1;
    retrievalCollapserTable[match + add + query][match + add
        + query] = collapse_compileout_add_p1;
    ...
    perceptualCollapserTable[empty][match + remove
        + query] = collapse_no_add_p1;
    ...
    // perceptualEvaluatorTable[match][add] = Evaluator.TRUE; -- is this buffer
    // stuffing...?
    ...
    perceptualCollapserTable[match][match + remove
        + query] = collapse_no_add_p1;
    ...
    perceptualCollapserTable[query][match + remove
        + query] = collapse_no_add_p1;
    ...
    perceptualCollapserTable[match
        + add][empty] = collapse_nonimmediate_add_p1_not_harvested;
    perceptualCollapserTable[match
        + add][query] = collapse_nonimmediate_add_p1_not_harvested;
    ...
    // perceptualEvaluatorTable[match+query][add] = Evaluator.TRUE; buffer
    // stuffing??
    ...
    perceptualEvaluatorTable[match + query][match + remove
        + query] = Evaluator.TRUE;
    ...
    perceptualCollapserTable[match + query][match
        + remove] = collapse_no_add_p1;
    perceptualCollapserTable[match + query][match + query] = collapse_no_add_p1;
    perceptualCollapserTable[match + query][match + remove
        + query] = collapse_no_add_p1;
    ...
    perceptualCollapserTable[match + add
        + query][empty] = collapse_nonimmediate_add_p1_not_harvested;
    perceptualCollapserTable[match + add
        + query][query] = collapse_nonimmediate_add_p1_not_harvested;
    ...
    perceptualEvaluatorTable[match + remove + query][add
        + query] = same_queries;
    ...
    perceptualCollapserTable[match + remove
        + query][empty] = collapse_no_add_p1;
    perceptualCollapserTable[match + remove + query][add] = collapse_no_add_p1;
    perceptualCollapserTable[match + remove
        + query][query] = collapse_no_add_p1;
    perceptualCollapserTable[match + remove + query][add
        + query] = collapse_no_add_p1;
  }
