...
public class RuntimeTracerRegistry extends
    AbstractExtensionPointRegistry<RuntimeTracerDescriptor>
{
  ...
  static private final transient Log      LOGGER  = LogFactory
                                                      .getLog(RuntimeTracerRegistry.class);

  static private final RuntimeTracerRegistry DEFAULT = new RuntimeTracerRegistry();

  static public RuntimeTracerRegistry getRegistry()
  {
    return DEFAULT;
  }
  ...
  @Override
  protected RuntimeTracerDescriptor createDescriptor(
      IPluginElement extPointElement)
  {
    if (extPointElement.getName().equals("tracer"))
    {
      ...
      if (LOGGER.isDebugEnabled())
        LOGGER.debug("Adding extension of instrument from "
            ...
            + parameters);
      return new RuntimeTracerDescriptor(extPointElement.getPluginBase()
          .getId(),
  ...
  {
    if (extPointElement.getName().equals("tracer"))
      return new RuntimeTracerDescriptor(extPointElement);
    else
      throw new IllegalArgumentException("Was expecting tracer tag, got "
          + extPointElement.getName());
  }
